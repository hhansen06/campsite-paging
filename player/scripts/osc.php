...
function music($enable)
{
    ...
    {
        echo "Enabling Music ...\n";
        send_osc("/strip/AutoMusik/Gain/Gain%20(dB)", "0.9");

    }
    else
    {
        echo "Disabling Music ...\n";
        send_osc("/strip/AutoMusik/Gain/Gain%20(dB)", "0.0");
    }
}

function send_osc($name, $value) {

    // "/strip/Playliste/Gain/Gain%20(dB)"
    $c = new OSCClient(); 
    $c->set_destination("mixer", 7890);
    $m1 = new OSCMessage($name);
    $m1->add_arg($value, "f");
